 ...
 * @version 1.0.1
 */
if ( !class_exists( 'WPSA_Notification' ) ) :
class WPSA_Notification extends WP_Error {
  /**
  ...
  public function build( $container_class = '' ) {
    ...
    foreach ( $this->errors as $code => $message ) {
      ...
      // container start tag
      $html .= "<div class=\"is-dismissible notice-$state notice $container_class $container_icon_class swift-notice \"><p>\n";
      // container icon
 ...
 * @since 1.0
 */
$wpsa_notifications = new WPSA_Notification();
/**
 ...
 * @return void        If at least one notification is present, echoes the notifications HTML.
 */
function wpsa_notifications( $thing = '' ) {
  global $wpsa_notifications;
  if ( is_object($thing) ) { $wpsa_notifications = $thing; }
  if ( !is_wpsa_notification( $wpsa_notifications ) ) { return false; }
  /**
   * Add a filter to change the default notifications container class.
   */
  $container_class = apply_filters( 'wpsa_container_class', $wpsa_notifications->container_class );
  /**
   * Build and display the notifications.
   */
  $wpsa_notifications->display( $container_class );
}
add_action( 'wpsa_notifications', 'wpsa_notifications', 10, 1 );
/**
 ...
 */
function is_wpsa_notification( $thing ) {
